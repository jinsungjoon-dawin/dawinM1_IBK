<script>
  ...
  async function getPerformcompositTitle() {
    const res = await fetch($rooturl + "/performcomposit/perfcomp_title?asisdt=" + conds.asisdt + "&tobedt=" + conds.tobedt);
    if (res.ok){
...
<div class="flex justify-between">
  <div class="w-3/12 bg-gray-700 rounded-lg flex-wrap p-3" >
    <div class="flex  border border-gray-100 rounded border-zinc-600 text-zinc-100 " on:click={() => { conds.asisdt=item.asisdt; conds.tobedt=item.tobedt; getPerformcompositTitle(); getPerformcompositList(); handleRowClick(idx);}}>
    ...
    {#if dates.length !== 0}
      {#each dates as item, idx}
          <div class="flex mb-3 border border-gray-100 rounded border-zinc-600 text-zinc-100 " on:click={() => { conds.asisdt=item.asisdt; conds.tobedt=item.tobedt; getPerformcompositTitle(); getPerformcompositList(); handleRowClick(idx);}}>
            ...
            <!-- <button class="w-1/6 bx bx-search-alt-2" on:click={() => { conds.asisdt=item.asisdt; conds.tobedt=item.tobedt; getPerformcompositTitle();}}>조회</button> -->
        </div>
      {/each}
    {/if}
  </div>
